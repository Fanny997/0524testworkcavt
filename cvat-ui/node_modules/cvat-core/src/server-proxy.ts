...
import {
    ...
    SerializedApiToken, APIApiTokensFilter, SerializedCustomOperation, CustomOperationsFilter,
...
function prepareCustomOperationData(details) {
    const data = new FormData();
    ...
    for (const [key, value] of Object.entries(details || {})) {
        ...
        if (Array.isArray(value)) {
            if (value.every((element) => isFileLike(element))) {
                value.forEach((element) => {
                    data.append(key, element);
                });
            } else {
                (data as any).set(key, JSON.stringify(value));
            }
        } else if (typeof value === 'object' && !isFileLike(value)) {
            (data as any).set(key, JSON.stringify(value));
        } else {
            (data as any).set(key, value);
        }
...
async function listCustomOperations(filter: CustomOperationsFilter = {}): Promise<SerializedCustomOperation[]> {
    const { backendAPI } = config;

    try {
        const response = await Axios.get(`${backendAPI}/custom-operations/definitions`, {
            params: {
                ...filter,
            },
        });
...
async function getCustomOperation(id: number): Promise<SerializedCustomOperation> {
    const { backendAPI } = config;

    try {
        const response = await Axios.get(`${backendAPI}/custom-operations/definitions/${id}`);
        return response.data;
    } catch (errorData) {
        throw generateError(errorData);
    }
...
async function createCustomOperation(data): Promise<SerializedCustomOperation> {
    const { backendAPI } = config;
    const body = prepareCustomOperationData(data);

    try {
        const response = await Axios.post(`${backendAPI}/custom-operations/definitions`, body, {
            headers: {
...
async function updateCustomOperation(id: number, data): Promise<SerializedCustomOperation> {
    const { backendAPI } = config;
    const body = prepareCustomOperationData(data);

    try {
        const response = await Axios.patch(`${backendAPI}/custom-operations/definitions/${id}`, body, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
...
async function deleteCustomOperation(id: number): Promise<void> {
    const { backendAPI } = config;

    try {
        await Axios.delete(`${backendAPI}/custom-operations/definitions/${id}`);
    } catch (errorData) {
        throw generateError(errorData);
    }
}

async function executeCustomOperation(id: number, data): Promise<any> {
    const { backendAPI } = config;
    const body = prepareCustomOperationData(data);

    try {
        const response = await Axios.post(`${backendAPI}/custom-operations/definitions/${id}/execute`, body, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
...
export default Object.freeze({
    ...
    customOperations: Object.freeze({
        list: listCustomOperations,
        get: getCustomOperation,
        create: createCustomOperation,
        update: updateCustomOperation,
        delete: deleteCustomOperation,
        execute: executeCustomOperation,
    }),
