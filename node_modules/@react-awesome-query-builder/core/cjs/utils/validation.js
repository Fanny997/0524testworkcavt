...
exports.sanitizeTree = exports.isValidTree = exports.getTreeBadFields = exports.getNewValueForFieldOp = exports.checkTree = exports._validateTree = void 0;
...
var _validateTree = exports._validateTree = function _validateTree(tree, _oldTree, config, oldConfig, options) {
  ...
  if (includeItemsPositions) {
    flatItems = (0, _treeUtils.getFlatTree)(fixedTree).items;
  }
  for (var id in meta.errors) {
    ...
    if (includeItemsPositions) {
      ...
      if (isDeleted) {
        ...
        if (!oldFlatItems) {
          oldFlatItems = (0, _treeUtils.getFlatTree)(tree).items;
        }
  ...
  for (var argKey in funcConfig.args) {
    ...
    if (argValue !== undefined) {
      ...
      if (canDropOrReset) {
        ...
        fixedValue = (0, _funcUtils.setFuncDefaultArg)(config, fixedValue, funcConfig, argKey);
      }
      if (!isValid) {
 ...
 * 
 */
var validateRange = exports.validateRange = function validateRange(config, field, operator, values, valueSrcs) {
...
var getNewValueForFieldOp = exports.getNewValueForFieldOp = function getNewValueForFieldOp(config) {
  ...
  var currentOperatorCardinality = currentOperator ? (0, _stuff.getOpCardinality)(currentOperatorConfig) : null;
  var operatorCardinality = newOperator ? (0, _stuff.getOpCardinality)(newOperatorConfig) : null;
  ...
  if (hasFieldChanged && _ruleUtils.selectTypes.includes(newType)) {
  ...
  for (var i = 0; i < operatorCardinality; i++) {
    ...
    var currentWidget = (0, _ruleUtils.getWidgetForFieldOp)(oldConfig, currentField, currentOperator, vs);
    var newWidget = (0, _ruleUtils.getWidgetForFieldOp)(config, newField, newOperator, vs);
    // need to also check value widgets if we changed operator and current value source was 'field'
    // cause for select type op '=' requires single value and op 'in' requires array value
    var currentValueWidget = vs === "value" ? currentWidget : (0, _ruleUtils.getWidgetForFieldOp)(oldConfig, currentField, currentOperator, "value");
    var newValueWidget = vs === "value" ? newWidget : (0, _ruleUtils.getWidgetForFieldOp)(config, newField, newOperator, "value");
  ...
  var valueSources = (0, _ruleUtils.getValueSourcesForFieldOp)(config, newField, newOperator, null);
  ...
  if (canReuseValue) {
    var _loop = function _loop(_i3) {
      ...
      var _validateValue5 = validateValue(config, newField, newField, newOperator, v, vType, vSrc, asyncListValues, canFix, isEndValue, canDropArgs),
        _validateValue6 = (0, _slicedToArray2["default"])(_validateValue5, 2),
      ...
      if (showError && firstError) {
        valueErrors[_i3] = (0, _i18n.translateValidation)(firstError);
      }
      if (willFix) {
  ...
  // build new values
  var newValue = new _immutable["default"].List(Array.from({
    length: operatorCardinality
  }, function (_ignore, i) {
    return valueFixes[i] !== undefined ? valueFixes[i] : canReuseValue ? currentValue.get(i) : undefined;
  }));
  var newValueSrc = new _immutable["default"].List(Array.from({
    length: operatorCardinality
  }, function (_ignore, i) {
  ...
  var newValueType = new _immutable["default"].List(Array.from({
    length: operatorCardinality
  }, function (_ignore, i) {
  ...
  var newValueError = new _immutable["default"].List(valueErrors);
  return {
